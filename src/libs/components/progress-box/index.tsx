      ...
      <div className="flex w-full flex-col py-7 md:flex-row md:items-start md:justify-start">
        {data?.map((item: any) => (
          <div className={styles.progress_container} key={item.id}>
            ...
            <div className="ml-5 flex flex-col items-start md:ml-0 md:items-center">
              {item && (
                <span className="text-left text-base font-semibold capitalize text-body-dark md:px-2 md:!text-center">
                  {item?.name}
                </span>
              )}
            </div>
